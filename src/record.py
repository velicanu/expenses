...
class CardRecord(object):
    ...
    def plot_categories(self):
        ...
        records_df[['category', 'amount']].groupby(
            'category')['amount'].sum(
            ).plot(x='category', y='amount', kind='bar', color=self.category_colors)


    def plot_over_time(self, freq='M', stacked=True):
        records_df = pd.DataFrame(strip_payment(self.records))
        records_df.index = pd.to_datetime(records_df['date'])
        records_df.groupby([pd.Grouper(freq='M'), 'category']).sum().reset_index().pivot(index='date', columns='category').plot(kind='bar', stacked=stacked)
